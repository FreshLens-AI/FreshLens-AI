...
- Isolation between vendors is enforced in PostgreSQL via RLS using pp.tenant_id set from the JWT.
...
| Tenant isolation         | Every business table has 	enant_id + RLS in the same migration |
| Redis namespacing        | Keys use 	enant:{tenant_id}:... |
